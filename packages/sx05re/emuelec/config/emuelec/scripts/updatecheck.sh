...
if test -e "/storage/.update/${UFILE}.sha256"; then
    ...
    if test "${DISTMD5}" = "${CURRMD5}"; then
        ...
        text_viewer -e -t "Invalid Checksum!" -f ${TEXT_VIEWER_FONT_SIZE} -m "invalid checksum. Got +${DISTMD5}+. Attempted +${CURRMD5}+.\n\n FORCE UPDATE ABORTED!"
        no_update
	exit 1
    fi
...
if [[ "$arguments" == *"forceupdate"* ]]; then
text_viewer -e -t "ERROR!" -f ${TEXT_VIEWER_FONT_SIZE} -m "The update file either does not exists or you are not connected to the internet! FORCE UPDATE ABORTED!\n\nEmulationstation will now restart!"
no_update
fi
    no_update
fi
...
if [[ "$arguments" == *"forceupdate"* ]]; then
    text_viewer -y -t "Force update!" -f ${TEXT_VIEWER_FONT_SIZE} -m "A forced update has been called, this will download the latest release depending on your type settings (stable or test/beta)\n\nThe system will reboot if the update file is downloaded succesfully.\n\nAre you sure you want to continue?"
    if [[ $? == 21 ]]; then
        forced_update
    else
