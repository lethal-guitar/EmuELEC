...
function copy_roms() {
    ...
    echo -en "\n\n\n$COPY" >> /tmp/display
    text_viewer -y -t "Copy roms from USB to SD" -f ${TEXT_VIEWER_FONT_SIZE} /tmp/display
	if [[ $? == 21 ]]; then
        rm /tmp/display
        ee_console disable
